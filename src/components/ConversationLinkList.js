...
function ConversationLinkList({ conversations, activeConversation, selectConversation, session, setCurrentRoute }) {
  return (
    <ul className="pl-3">
      ...
      {conversations.map((conversation, index) => {
        return <ConversationLinkListItem key={index} selectConversation={selectConversation} conversation={conversation} isActive={activeConversation._id == conversation._id} setCurrentRoute={setCurrentRoute} />
      })}
    </ul>
  );
};
