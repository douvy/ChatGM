...
function ChatMessage({
  ...
  return (
    ...
    >
      <div
        className={`message p-4 pt-4 relative ${
          message.id == referencedMessage?.id ? 'active' : ''
        }`}
      >
        <img
          src={sender.avatarSource || avatarSource}
          alt='Avatar'
          className='w-9 h-9 rounded-full absolute left-4 top-2'
        />
        <div className='pl-16 pt-0'>
          <span className='text-sm mb-1 inline-block name'>{sentFrom}</span>{' '}
          <br />
          <span className='text-xs inline-block absolute top-3 right-4 timestamp'>
            ...
            <i
              className={`fa-stars ${
                localMessage.starred ? 'fa-solid' : 'fa-regular'
              } ml-2 text-yellow cursor-pointer`}
              onClick={starMessage}
            ></i>
            {copied ? (
            ...
            {userInfo.enableChatGMBot && userInfo.telegramUserId && (
              <i
                className='fa fa-telegram text-lightblue w-5 h-5 ml-3'
                onClick={async e => {
                  e.stopPropagation();
          ...
          {!editingMessage ? (
            ...
            <form>
              ...
              <textarea
                ...
                ref={textareaRef}
                className='p-0 m-0 mt-5 h-auto w-full border-none focus:outline-none'
                rows='1'
                autoFocus={true}
                spellCheck='false'
                onInput={e => {
